...
// Function to add events using AJAX
function addEvent() {
    ...
    if (date && title) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                fetchEvents();  // Fetch updated events after adding
            }
        };
...
// Function to fetch events from the database using AJAX
function fetchEvents() {
    let xhr = new XMLHttpRequest();
    xhr.open("GET", "get_events.php", true);
    xhr.onload = function () {
        if (xhr.status === 200) {
            events = JSON.parse(xhr.responseText); // Load events from the database
...
// Call the fetchEvents function to initially load events from the database
fetchEvents();
